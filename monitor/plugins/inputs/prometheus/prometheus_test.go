...
func TestPrometheus_Collect(t *testing.T) {
	config := `{"addresses":["http://127.0.0.1:9101/metrics"], "httpTimeout":"10s"}`
	...
	go func() {
		...
		if err != nil {
			t.Errorf("http listenAndServer failed, err %s", err.Error())
		}
	}()
	...
	metrics, err := p.CollectMsgs(context.Background())
	if err != nil {
		t.Errorf("collect prometheus data err %s", err.Error())
	}
...
# HELP node_exporter_build_info A message with a constant '1' value labeled by version, revision, branch, and goversion from which node_exporter was built.
...
# HELP promhttp_metric_handler_errors_total Total number of internal errors encountered by the promhttp message handler.
